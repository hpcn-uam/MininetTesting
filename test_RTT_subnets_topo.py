...
""" Script that tests RTT in a mininet deployment by using ping tool

Usage: ./test_RTT_subnets_topo.py [Options]
...
arpTable={}
def perfTest(verbose, output_path):
	...
	for i in range(0, S):
		...
		for j in range(0, H/S):
			...
			#if even number of hosts in sub net last hosts is both server and client
	...
	for elem in clients:
		...
		command = 'ping '+ str(serv.IP())+' &> '+output_path + str(elem.IP())+'/'+ 'client_' +str(elem.IP()) + ' &'
